...
(function () {
  ...
  if (trElementsList.length > 1) {
    ...
    for (let i = 0; i < itemChildrenList.length; i++) {
      if (
        itemChildrenList[i].innerText != "Modem GSM" &&
        itemChildrenList[i].innerText != "Modem DSL" &&
        itemChildrenList[i].innerText != "Switch" &&
        itemChildrenList[i].innerText != "Brak połączenia" &&
        itemChildrenList[i].innerText != "Joby SQL"
      ) {
        audio.play();
        createPopup();
        break;
