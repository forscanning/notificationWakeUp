...
(function () {
  ...
  const trElementsList = [
    ...document.querySelectorAll(
      "#ctl00_PlaceHolderMain_Tabs_tabNiePodjete_GridZglNieprzydzielone tbody > tr"
    ),
  ];
  ...
  if (trElementsList.length >= 1) {
    ...
    for (let i = 0; i < trElementsList.length; i++) {
      if (
        itemChildrenList[i].category !== "Modem GSM" &&
        itemChildrenList[i].category !== "Modem DSL" &&
        itemChildrenList[i].category !== "Switch" &&
        itemChildrenList[i].category !== "Brak połączenia" &&
        itemChildrenList[i].category !== "Joby SQL"
      ) {
        audio.play();
        createPopup();
        break;
    ...
    if (
      typeof getCookie("numberOfQuest") === "undefined" ||
      getCookie("numberOfQuest") <= 0
    ) {
      createCookie("numberOfQuest", notAlarmList.length, 300);
    }

    if (
      notAlarmList.length >= 1 ||
      typeof getCookie("numberOfQuest") !== "undefined"
    ) {
      if (getCookie("numberOfQuest") !== 0) {
        ...
        if (getCookie("numberOfQuest") > 0) {
          window.open(
            "http://e-ross/_layouts/15/ItApp/sd/EventList.aspx",
            "_blank"
          );
          createCookie("numberOfQuest", cookie - 1, 300);
          window.open(
            itemChildrenList[getCookie("numberOfQuest")].href,
            "_self"
          );
        }
      }
